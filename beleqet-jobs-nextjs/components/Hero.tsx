...
import {
  ArrowRight, CheckCircle2, MapPin, Search, Sparkles, TrendingUp,
} from "lucide-react";
        ...
        <div className="relative z-10">
          ...
          >
            <label className="flex items-center gap-3 rounded-2xl px-3 py-3">
              ...
              <span className="sr-only">{t("jobTitleLabel")}</span>
              <input
                value={query}
            ...
            >
              {t("searchButton")} <ArrowRight className="h-4 w-4" />
            </button>
          </form>

          <div className="mt-5 flex flex-wrap items-center gap-2 text-xs text-muted">
            <span className="font-semibold text-ink">{t("trending")}</span>
        ...
        <div className="relative mx-auto hidden w-full max-w-[520px] lg:block">
          ...
          <div className="relative rotate-2 rounded-[36px] bg-primary p-6 shadow-[0_35px_80px_rgba(4,22,3,.2)]">
            <div className="flex items-center justify-between border-b border-white/10 pb-5">
              <div>
                <p className="text-xs font-semibold text-white/50">{t("recommended")}</p>
                <p className="mt-1 text-xl font-bold text-white">{t("freshOpportunities")}</p>
              </div>
            ...
            <div className="mt-5 space-y-3">
              <Opportunity title="Senior Product Designer" company="Kifiya Financial Technology" meta="Addis Ababa · Hybrid" tone="bg-[#d8ff3e]" initials="KF" />
              <Opportunity title="Frontend Engineer" company="PRAGMA Investment" meta="Remote · Full time" tone="bg-[#ffbc80]" initials="PI" />
              <Opportunity title="Marketing Lead" company="Dodai Manufacturing" meta="Addis Ababa · Full time" tone="bg-[#b8e5ff]" initials="DM" />
            </div>
            <Link
              href="/jobs"
          ...
          <div className="absolute -bottom-10 -left-16 -rotate-3 rounded-2xl border border-primary/10 bg-white p-4 shadow-xl">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted">{t("profileViews")}</p>
              </div>
            </div>
          </div>
          <div className="absolute -right-8 top-24 rotate-6 rounded-2xl bg-white p-3 shadow-xl">
            <div className="flex items-center gap-2 text-xs font-bold text-primary">
              <CheckCircle2 className="h-5 w-5 text-brandGreen" /> {t("verifiedEmployers")}
            </div>
          </div>
        </div>
      </div>
...
function Opportunity({ title, company, meta, tone, initials }: {
  title: string; company: string; meta: string; tone: string; initials: string;
}) {
  return (
    <div className="rounded-[20px] bg-white p-4">
      <div className="flex gap-3">
        <span className={`inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-xl text-xs font-black text-primary ${tone}`}>
          {initials}
        </span>
