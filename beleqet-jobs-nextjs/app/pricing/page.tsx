...
function planFeatureList(plan: Plan): string[] {
  ...
  return Object.entries(plan.features).map(([key, value]) => {
    if (typeof value === 'boolean')
      return value ? humanizeKey(key) : `No ${humanizeKey(key).toLowerCase()}`;
    ...
    <div className="min-h-screen bg-[#f7f5ef]">
                  ...
                  <ul className="flex-1 space-y-3 p-7">
                    {features.map((f) => (
                      <li key={f} className="flex items-start gap-2.5 text-sm">
                        ...
                        <span className={presentation.highlight ? 'text-white/80' : 'text-ink/80'}>
                          {f}
                        </span>
                      </li>
                    ))}
      ...
      <section className="container-page pb-20">
        <div className="mx-auto max-w-3xl overflow-hidden rounded-3xl bg-primary px-8 py-12 text-center text-white shadow-[0_20px_60px_-15px_rgba(4,22,3,0.4)]">
          ...
          <h2 className="mt-3 text-3xl font-black">Start on the Free plan, upgrade anytime</h2>
          <p className="mx-auto mt-4 max-w-md text-sm text-white/60 leading-relaxed">
            No contracts, no commitments. Cancel a paid plan whenever you like — your access
            continues until the period you already paid for ends.
          </p>
