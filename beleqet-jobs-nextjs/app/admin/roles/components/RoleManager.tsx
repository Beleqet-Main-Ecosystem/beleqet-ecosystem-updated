  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {roles.map((role) => (
          <div key={role.id} className="group relative bg-slate-800/80 rounded-2xl border border-slate-700 p-6 flex flex-col justify-between overflow-hidden transition-all hover:-translate-y-1 hover:shadow-xl hover:shadow-indigo-900/20 hover:border-indigo-500/50">
            {/* System Badge */}
            {role.isSystem && (
              ...
              <div className="space-y-2 mt-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {role.permissions.map(p => (
                    <span key={p.id} className="px-2.5 py-1 rounded-md bg-slate-700/50 border border-slate-600/50 text-xs text-slate-300 hover:bg-slate-600 transition-colors cursor-default" title={p.description}>
                      {p.action}
                    </span>
                  ))}
            ...
            <div className="mt-8 pt-4 border-t border-slate-700/50 flex justify-end space-x-3 opacity-80 group-hover:opacity-100 transition-opacity">
              <button className="text-slate-400 hover:text-indigo-400 transition-colors text-sm font-medium">Edit</button>
              {!role.isSystem && (
                <button className="text-slate-400 hover:text-red-400 transition-colors text-sm font-medium">Delete</button>
              )}
            </div>
          </div>
        ))}
