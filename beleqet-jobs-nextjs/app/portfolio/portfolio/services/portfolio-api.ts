...
import { PORTFOLIO_API_PATH, PORTFOLIO_DRAFT_KEY } from '../constants/storage';
 ...
 */
export async function savePortfolioToApi(data: MasterProfileData): Promise<boolean> {
...
export function saveLocalPortfolioDraft(data: MasterProfileData): void {
  ...
  localStorage.setItem(PORTFOLIO_DRAFT_KEY, JSON.stringify(touchPortfolio(data)));
}
