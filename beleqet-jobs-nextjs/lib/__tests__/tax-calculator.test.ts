...
function buildCalculatePayload(grossInput: string, countryCode: CountryCode, currency: Currency) {
...
describe('Tax calculator client utilities', () => {
  ...
  describe('CurrencyUtil float ↔ smallest unit', () => {
    ...
    it('round-trips decimal → smallest → decimal', () => {
      ...
      for (const decimal of samples) {
        ...
        expect(CurrencyUtil.toDecimal(smallest)).toBeCloseTo(Math.round(decimal * 100) / 100, 2);
      }
    });

    it('throws on invalid toSmallestUnit input', () => {
      expect(() => CurrencyUtil.toSmallestUnit(NaN)).toThrow(/must be a number/i);
  ...
  describe('CurrencyUtil.format UI display', () => {
    ...
    it('formats result rows consistently with API integer fields', () => {
      ...
      expect(CurrencyUtil.format(apiResult.taxAmount, apiResult.currency)).toContain('24,540.00');
      expect(CurrencyUtil.format(apiResult.netIncome, apiResult.currency)).toContain('95,460.00');
