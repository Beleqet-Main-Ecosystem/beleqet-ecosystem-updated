...
describe('EncryptionService', () => {
  ...
  it('should handle key with length not equal to 64 but still 32 bytes when parsed or throw if key is not valid hex', () => {
    const longConfig = {
      get: jest
        .fn()
        .mockReturnValue(
          '0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdefextraaaaa',
        ),
    };
  ...
  it('should throw error when key length is invalid (not 32 bytes)', () => {
    ...
    expect(() => new EncryptionService(shortConfig as any)).toThrow(
  ...
  it('should throw error when key is missing', () => {
    ...
    expect(() => new EncryptionService(missingConfig as any)).toThrow(
