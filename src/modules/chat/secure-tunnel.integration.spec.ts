...
@Global()
@Module({
  providers: [{ provide: I18nService, useValue: { t: jest.fn((key: string) => key) } }],
  exports: [I18nService],
})
...
describe('Secure Tunnel — Integration Tests', () => {
  ...
  describe('KeysService ↔ PrismaService (User DB)', () => {
    ...
    it('should fetch a public key by user ID (cross-module lookup)', async () => {
      ...
      const mockRecord = {
        id: 'key-002',
        userId,
        publicKey: 'encrypted:recipientBase64Key==',
        createdAt: new Date(),
        updatedAt: new Date(),
      };
  ...
  describe('ChatService ↔ ChatParticipant (Security Module integration)', () => {
    ...
    it('should reject message persistence for a non-participant (security boundary)', async () => {
      ...
      await expect(
        chatService.saveMessage('room-x', 'intruder-id', 'ciphertext==', {
          encrypted: true,
          iv: 'iv',
        }),
      ).rejects.toThrow();
  ...
  describe('E2EE data integrity (GDPR compliance)', () => {
    it('should store metadata with encryption flag and IV — not plaintext', async () => {
      ...
      const simulatedCiphertext = btoa(
        plaintext
          .split('')
      ...
      const result = await chatService.saveMessage(roomId, senderId, simulatedCiphertext, {
        encrypted: true,
        iv: simulatedIv,
      });
