...
let NotificationsProcessor = NotificationsProcessor_1 = class NotificationsProcessor extends bullmq_1.WorkerHost {
    constructor(prisma, config) {
        ...
        this.transporter = nodemailer.createTransport({
            ...
            auth: {
                ...
                pass: this.config.get('SMTP_PASSWORD') ?? this.config.get('SMTP_PASS'),
            },
    ...
    async process(job) {
        switch (job.name) {
            case queues_constants_1.NOTIFICATION_JOBS.SEND_IN_APP:
                await this.sendInApp(job);
                break;
            case queues_constants_1.NOTIFICATION_JOBS.SEND_TELEGRAM:
                await this.sendTelegram(job);
                break;
            case queues_constants_1.NOTIFICATION_JOBS.SEND_EMAIL:
        ...
        try {
            await this.transporter.sendMail({
                from: this.config.get('SMTP_FROM') ??
                    this.config.get('EMAIL_FROM', 'Beleqet <noreply@beleqet.com>'),
                to,
                subject,
                html,
                text,
...
exports.NotificationsProcessor = NotificationsProcessor = NotificationsProcessor_1 = __decorate([
    ...
    (0, bullmq_1.Processor)(queues_constants_1.QUEUE_NAMES.NOTIFICATIONS),
