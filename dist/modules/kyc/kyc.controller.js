...
let KycController = class KycController {
    ...
    async submitKyc(files, user, dto) {
        ...
        if (documentFile.buffer.length > 5 * 1024 * 1024 ||
            faceScanFile.buffer.length > 5 * 1024 * 1024) {
            throw new common_1.BadRequestException('File size exceeds the maximum limit of 5MB.');
        }
        if (!documentFile.mimetype.match(/^image\/(jpeg|png|webp|gif)$/) ||
            !faceScanFile.mimetype.match(/^image\/(jpeg|png|webp|gif)$/)) {
            throw new common_1.BadRequestException('Only image files (JPEG, PNG, WEBP, GIF) are allowed for KYC verification.');
        }
    ...
    (0, common_1.UseInterceptors)((0, platform_express_1.FileFieldsInterceptor)([
        ...
        fileFilter: (req, file, callback) => {
            if (!file.mimetype.match(/^image\/(jpeg|png|webp|gif)$/)) {
                return callback(new common_1.BadRequestException(`Only image files (JPEG, PNG, WEBP, GIF) are allowed for KYC verification. Got invalid type: ${file.mimetype}`), false);
            }
            callback(null, true);
        },
    })),
